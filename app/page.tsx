...
export default function Home() {
  ...
  const counts = Object.fromEntries(domains.map((d) => { const list = scenesIn(d.id); return [d.id, { scenes: list.length, demos: list.reduce((n, s) => n + s.demos.length, 0), first: list[0]?.id ?? "" }]; }));
  return (
    <main className="shell">
      <section className="hero">
        ...
        <ul className="hero-stats" aria-label="Platform at a glance">
          <li><b>{demoCount}</b><span>demos, zero redirects</span></li>
          <li><b>{scenes.length}</b><span>scenes, ≤ 3 demos each</span></li>
          <li><b>{libCount}</b><span>open-source libraries and engines credited</span></li>
        </ul>
      </section>
      ...
      <section className="domains" id="domains" aria-labelledby="domains-title">
        ...
        <div className="domain-grid">
          {domains.map((d, i) => (
            <Link href={`/${d.id}`} className="domain-card live-card reveal" key={d.id} style={{ ["--hue" as string]: d.hue, animationDelay: `${i * 50}ms` }}>
              <div className="dc-preview"><Preview kind={d.id} seed={i + 2} /></div>
              <div className="dc-body">
                <h3>{d.name}</h3>
                <p>{d.blurb}</p>
              </div>
              <div className="dc-foot">
                <span>{counts[d.id].scenes} scenes · {counts[d.id].demos} demos</span>
      ...
      <section className="method-strip" aria-labelledby="method-title">
        <div className="method-intro">
          ...
          <Link href="/methodology" className="btn">Read the method</Link>
        </div>
        <ol className="pipeline">
          {[["Question", "“Which candidate trajectory did the planner pick, and why?”"], ["Data contract", "Typed inputs, units, synthetic vs recorded"], ["Engine", "Planner, MuJoCo, deck.gl, LOB, transformer…"], ["View", "3D stage, BEV inset, plots, maps, timelines"], ["Evidence", "Telemetry strip, JSON export, stated assumptions"]].map(([t, s]) => (
